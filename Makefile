...
SRCS := async.c audio.c clargs.c drawer.c driver.c emit.c \
	...
	matrix.c mutator.c particle.c plimits.c plist.c \
...
CFLAGS_READLINE =
LDFLAGS_READLINE =
ifeq ($(DISABLE_READLINE),)
CFLAGS_READLINE = $(call pkg_cflags,readline)
LDFLAGS_READLINE = $(call pkg_ldflags,readline)
ifneq ($(CFLAGS_READLINE),)
CFLAGS += -DHAVE_READLINE $(CFLAGS_READLINE)
